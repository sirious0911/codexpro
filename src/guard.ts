...
export class WorkspaceManager {
  ...
  getWorkspace(id?: string): Workspace {
    ...
    let workspace = this.workspaces.get(id) ?? this.registry.get(id);
    if (workspace) {
      try {
        const stat = fs.statSync(workspace.root);
        const realRoot = fs.realpathSync.native(workspace.root);
        const allowed = stat.isDirectory() && this.config.allowedRoots.some((allowedRoot) => isSubpath(realRoot, allowedRoot));
        if (!allowed || workspaceIdForRoot(realRoot) !== id) {
          throw new Error("stale workspace root");
        }
        this.workspaces.set(id, workspace);
      } catch {
        this.workspaces.delete(id);
